...
function renderLogistica(){
  ...
  var gmvT,ffV,xdV,ssV;
  if(pc.gran==='daily'){var _s=pc.curr[0],_e=pc.curr[1];gmvT=sumDailyFromData(RAW.logistica_daily,_s,_e,'gmv_total')||1;ffV=sumDailyFromData(RAW.logistica_daily,_s,_e,'gmv_ff');xdV=sumDailyFromData(RAW.logistica_daily,_s,_e,'gmv_xd');ssV=sumDailyFromData(RAW.logistica_daily,_s,_e,'gmv_ss');}
  else{gmvT=sumMeses(allM,meses,'gmv_total')||1;ffV=sumMeses(allM,meses,'gmv_ff');xdV=sumMeses(allM,meses,'gmv_xd');ssV=sumMeses(allM,meses,'gmv_ss');}
  var ffP=(ffV/gmvT)*100,xdP=(xdV/gmvT)*100,ssP=(ssV/gmvT)*100;
  var ffK=pc.gran==='daily'?{value:ffV,d1:null,d2:null}:computeKPI(pc,allM,'gmv_ff',RAW.logistica_daily);
